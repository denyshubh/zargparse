print_help = (conf) => {
  ...
  for (let e of conf.options) {
    ...
    const estr = `\t --${e.vname}${e.name ? ', -'+e.name : ''} \t ${e.help} ${e.mandatory ? '(mandatory)': ''}`;
    opt_str.push(estr);
  }
...
argparse = (args, conf) => {
  ...
  for (let i=0; i<args.length; i++) {
    const s = args[i];
    let key, value; // key-value to match
  ...
  // Checking mandatory arguments ----- //
  const mandatory = conf.options.filter(e => e.mandatory);
